...
colMix = SimCol(obj);
showCol(colMix)
[X_no_dither,map]= rgb2ind(double(colMix),n_colours ,'nodither');
dip_image(X_no_dither)


figure, imshow(X_no_dither,map);
rgbImage = ind2rgb(X_no_dither, map);
Iav = dip_image(rgbImage)
...
I_mask = dip_image(ones(size(I_mask)));
...
for opdfact = 3;%linspace(2,4,20)
    ...
    [OPDaxis,R,G,B]=getCalibration(obj,I_mes,n_colours);
