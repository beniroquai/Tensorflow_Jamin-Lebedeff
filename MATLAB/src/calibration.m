...
% correct for hot pixels (simply pixel that are white - I know bad idea!)
% replace by neighbouring pixel
%hotpixel_val = max(Iav)*.8;
%Iav(find(Iav>hotpixel_val)) = Iav(find(Iav>hotpixel_val)+1);
...
if(0)
    ...
    fh=dipshow(I_subtract(:,:,1)); % find edges of CC signal
    diptruesize(fh, 100);
    %fh=dipshow(abs(AllSubtracafftFT(:,:,0))^.1) % find edges of CC signal
...
for opdfact = 3;%linspace(2,4,20)
    ...
    [OPDaxis,R,G,B]=getCalibration(obj,I_mes,256);
...
%% apply to real data
Iob_file_name = 'IMG_20170721_151017.dng';
