...
mysize=size(B); % make it a square
mysize=mysize(1)
A = cat(3, extract(B(:,:,0), [mysize(1) mysize(1)], []), extract(B(:,:,1), [mysize(1) mysize(1)], []), extract(B(:,:,2), [mysize(1) mysize(1)], []));
...
crosscorr_res = real(ift(conj(ft(obj)).*ft(A(:,:,0))));
q = maxima(crosscorr_res, 2)*crosscorr_res;
g = q > 5e4; % check for ghost images!!
...
pix_r = reshape(A(:,:,0), num_pixel , 1);
pix_g = reshape(A(:,:,1), num_pixel , 1);
pix_b = reshape(A(:,:,2), num_pixel , 1);
